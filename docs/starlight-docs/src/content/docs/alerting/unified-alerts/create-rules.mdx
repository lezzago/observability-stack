...
5. **Add a trigger** — a name, severity, and condition (see [Trigger conditions](#trigger-conditions)). Optionally add notification actions.
...
Because [SLOs](/docs/slo/) deploy their burn-rate alerts as Prometheus rules, SLO breaches show up here as **Metric** rules too. Metric-rule alerts are delivered through the Prometheus Alertmanager [routing tree](/docs/alerting/unified-alerts/explore-rules/#routing).
