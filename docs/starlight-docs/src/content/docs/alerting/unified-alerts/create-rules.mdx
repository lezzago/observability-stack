...
A **log rule** alerts on data indexed in OpenSearch — your logs and traces. You write the query in **PPL** (Piped Processing Language), the same piped language you use in [Discover](/docs/investigate/) and the [PPL query builder](/docs/ppl/query-builder/), and the rule fires when the query's results meet a condition you set.
...
5. **Add a trigger** — a name, severity, and condition (see [Trigger conditions](#trigger-conditions)). Optionally add [notification actions](/docs/alerting/unified-alerts/notifications/#log-rule-notifications-trigger-actions) to send the alert to Slack, email, or a webhook.
...
Metric-rule alerts are delivered through the Prometheus Alertmanager — set up where they go in [Notification Channels](/docs/alerting/unified-alerts/notifications/#metric-rule-notifications-alertmanager), and view the resulting [routing tree](/docs/alerting/unified-alerts/explore-rules/#routing). Because [SLOs](/docs/slo/) deploy their burn-rate alerts as Prometheus rules, SLO breaches show up here as **Metric** rules too.
